...
test("scores API shapes the live feed for the browser", async () => {
  ...
  globalThis.fetch = async (url) => {
    ...
    events: [{
      ...
      date: "2026-07-09T04:00Z",
      endDate: "2026-07-12T04:00Z",
      competitions: [{
        competitors: [{
          id: "10140",
          order: 2,
          score: "-3",
          athlete: { displayName: "Max Homa", shortName: "M. Homa" },
          linescores: [
  ...
  try {
    await scoresHandler({ method: "GET", url: "/api/scores?tournament=isco-championship", headers: { host: "example.test" } }, response);
  } finally {
    globalThis.fetch = originalFetch;
  }

  assert.equal(statusCode, 200);
  assert.ok(fetchedUrl.includes("event=401811956"));
  assert.equal(body.event.id, "401811956");
  assert.equal(body.event.slug, "isco-championship");
  assert.equal(body.event.currentRound, 2);
  assert.equal(body.players[0].name, "Max Homa");
