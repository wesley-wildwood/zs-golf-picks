...
function shapePlayer(competitor, currentRound) {
  ...
  return {
    ...
    missedCut: hasMissedCut(competitor),
...
function hasMissedCut(competitor) {
  const statusText = [
    competitor.status?.type?.name,
    competitor.status?.type?.description,
    competitor.status?.type?.abbreviation,
    competitor.status?.displayValue,
    competitor.status?.description,
    competitor.status?.shortDetail,
    competitor.status?.detail,
    competitor.rank?.displayValue,
    competitor.rank?.shortDisplayValue,
    competitor.position?.displayValue,
    competitor.position?.shortDisplayValue,
    competitor.displayValue
  ].filter(Boolean).join(" ");

  return /\b(mc|cut|missed cut)\b/i.test(statusText);
}
