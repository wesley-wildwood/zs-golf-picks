...
This package mirrors the Vercel + Supabase deployment pattern used by the other leaderboard build. Vercel serves the static site and `/api/scores`; Supabase stores timestamped score snapshots for the Wyndham Championship leaderboard.
...
```text
DEFAULT_TOURNAMENT_SLUG=wyndham-championship
```

The app already defaults to the Wyndham Championship, and the browser requests:

- `/api/scores?tournament=wyndham-championship`
...
2. Confirm the page shows **Wyndham Championship**.
...
6. Visit `https://YOUR-SITE.vercel.app/api/scores?tournament=wyndham-championship` and confirm JSON loads with `event`, `players`, and `updatedAt`.
7. Confirm the JSON has `event.slug` set to `wyndham-championship`.
8. Confirm the JSON field `snapshotSaved` is `true`.
9. In Supabase, open **Table Editor -> score_snapshots** and confirm saved rows appear with `event_slug` set to `wyndham-championship`.
10. Ensure Vercel **Deployment Protection** is disabled for Production if everyone should have access.

## Troubleshooting

- **Site still shows U.S. Open:** Confirm GitHub has the new `public/scoring.js` with `eventId: "401811961"` and `DEFAULT_TOURNAMENT_SLUG = "wyndham-championship"`, then confirm Vercel's latest Production deployment was built from that commit and Root Directory is `./`.
- **Scores delayed:** Open `/api/scores?tournament=wyndham-championship` directly, then inspect **Vercel -> Project -> Logs**.
...
- **Wrong ESPN event:** If `/api/scores?tournament=wyndham-championship` returns a different tournament, update `eventId` in `public/scoring.js`, commit, and redeploy.
